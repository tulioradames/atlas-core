 ...
 *
 * V 1.3.3: Web App exclusivo de Expansões com importação por link de Drive externo. Use este script na conta proprietária do Drive.
 */
...
function validarModuloPermitidoAtnx(body) {
  ...
  const moduloEfetivo = informado || 'documentacao';
  if (moduloEfetivo !== permitido) {
    throw new Error('Este Web App pertence ao módulo ' + permitido + ', mas recebeu uma requisição do módulo ' + moduloEfetivo + '. Configure a URL correta no Atlas.');
  }
}
...
function escanearPastaDriveDados(body) {
  const entrada = String(body.folderUrl || body.folderId || '').trim();
  const scope = String(body.scope || 'auto').toLowerCase().trim();
  const pastaBase = entrada ? DriveApp.getFolderById(extrairIdPastaDrive(entrada)) : obterPastaRaiz();
...
function escanearExpansoesObrasPorLinkDados(body) {
  const entrada = String(body.folderUrl || body.folderId || body.driveLink || '').trim();
  if (!entrada) throw new Error('Informe o link ou ID da pasta de terceiros para importar.');

  const pastaBase = DriveApp.getFolderById(extrairIdPastaDrive(entrada));
  ...
  return {
    ...
    estruturaEsperada: 'Pasta raiz / Obra / Elemento / Fotos / imagens. Exemplo: FOTOS EXECUÇÃO BNDS / Bananeiras_PB / 144FO-BNN-R1-CD1 / Fotos.'
  };
}
...
function extrairIdPastaDrive(valor) {
  ...
  throw new Error('Não foi possível identificar o ID da pasta do Google Drive.');
}
